...
class CloneOpSpec extends MultiGitOpSpec {
  ...
  def 'clone with default settings clones as expected'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri)
    then:
  ...
  def 'clone with different remote does not use origin'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, remote: 'oranges')
    then:
  ...
  def 'clone with bare true does not have a working tree'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, bare: true)
    then:
  ...
  def 'clone with checkout false does not check out a working tree'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, checkout: false)
    then:
  ...
  def 'clone with checkout false and refToCheckout set fails'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, checkout: false, refToCheckout: 'branch2')
    then:
  ...
  def 'clone with refToCheckout set to simple branch name works'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, refToCheckout: 'branch1')
    then:
  ...
  def 'clone with refToCheckout set to simple tag name works'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, refToCheckout: 'tag1')
    then:
  ...
  def 'clone with refToCheckout set to full ref name works'() {
    when:
    def grgit = Git.clone(dir: repoDir, uri: remoteUri, refToCheckout: 'refs/heads/branch2')
    then:
